        ...
        if current_lock.locked():
            if bet_message:
                raise commands.CheckFailure(
                    f"You can't bet multiple times at once - finish your current game first! (<{bet_message.jump_url}>)",
                )
            raise commands.CheckFailure(
                f"You can't bet multiple times at once - finish your current game first! (could not find message)",
            )
